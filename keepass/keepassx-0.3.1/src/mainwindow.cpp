...
void KeepassMainWindow::setupConnections(){
	...
	connect(EditURLToClipboardAction, SIGNAL(triggered()), EntryView, SLOT(OnURLToClipboard()));
	connect(OpenSSH, SIGNAL(triggered()), EntryView, SLOT(OnOpenSSH() ));
	connect(OpenFTP, SIGNAL(triggered()), EntryView, SLOT(OnOpenFTP() ));
	...
	connect(QuickSearchEdit,SIGNAL(cursorPositionChanged(int,int)), this, SLOT(OnQuickSearch()));
