...
void KeepassEntryView::OnURLToClipboard() {
	if (selectedItems().size() == 0) return;
	Clipboard->setText(((EntryViewItem*)selectedItems().first())->EntryHandle->url(),  QClipboard::Clipboard);
	if(Clipboard->supportsSelection()){
		Clipboard->setText(((EntryViewItem*)selectedItems().first())->EntryHandle->url(),QClipboard::Selection);
	}
	ClipboardTimer.setSingleShot(true);
	ClipboardTimer.start(config->clipboardTimeOut()*1000);
...
void KeepassEntryView::OnOpenSSH() {
	if( selectedItems().size() == 0 ) return;
	IEntryHandle* i = ((EntryViewItem*)selectedItems().first())->EntryHandle;
	SecString password = i->password();
	password.unlock();

	openBrowser( "cmd:///etc/alternatives/x-terminal-emulator -e \"sshpass -p '"+password.string()+"' ssh '"+i->username()+"@"+i->url()+"'\"" );
}

void KeepassEntryView::OnOpenFTP() {
	if( selectedItems().size() == 0 ) return;
	IEntryHandle* i = ((EntryViewItem*)selectedItems().first())->EntryHandle;
	SecString password = i->password();
	password.unlock();
